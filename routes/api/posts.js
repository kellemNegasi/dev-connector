...
router.post(
  ...
  (req, res) => {
    ...
    NewPost.save()
      ...
      .catch((err) =>
        res.status(404).json({ error: "failed to save the post" })
      );
  }
);
...
router.get("/", (req, res) => {
  Post.find()
    ...
    .catch((err) =>
      res.status(404).json({ error: "failed to retrieve all posts" })
    );
});
...
router.get("/:id", (req, res) => {
  Post.findById(req.params.id)
    ...
    .catch((err) =>
      res
        .status(404)
        .json({ error: "failed to retrieve a post by the given id" })
    );
});
          ...
          .then((post) => {
            ...
            post
              ...
              .catch((err) =>
                res
                  .status(404)
                  .json({ savingfaild: "failed to save the like to database" })
              );
          })
          .catch((err) =>
            res.status(404).json({ postnotfound: "post not found" })
          ...
          .then((post) => {
            if (
              ...
              post
                ...
                .then((post) => res.json(post))
                .catch((err) =>
                  res.status(404).json({ error: "saving error" })
                );
            }
          })
          .catch((err) =>
